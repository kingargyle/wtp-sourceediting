...
public class NodeHelper {
		...
		while(regionIterator.hasNext() ){
			   ...
			   if(r.getType()==DOMRegionContext.XML_TAG_ATTRIBUTE_NAME){
				int start = r.getStart();
			   	...
			   	if(isInArray(name, tagname)) return true;
			   }
		}	
		return false;	
	}
		...
		while(regionIterator.hasNext()  ){
			 ...
			 if(r.getType()==DOMRegionContext.XML_TAG_ATTRIBUTE_NAME){
					int start = r.getStart();
				   	...
				   	String tagname = region.getText().substring(start, offset).trim();
				   	/* Attribute values aren't case sensative, also make sure next region is attrib value */
				   	if(tagname.equalsIgnoreCase(name)) { 
				   		if(regionIterator.hasNext() ) regionIterator.next();
				    	if(regionIterator.hasNext() ) r = ((ITextRegion)regionIterator.next());
	...
	public String AttrToString(){
		...
		while(regionIterator.hasNext() ) {
			    ...
			    if(r.getType()==  DOMRegionContext.XML_TAG_ATTRIBUTE_NAME){
			    	...
			    	/* Theres a XML_TAG_ATTRIBUTE_EQUALS after the XML_TAG_ATTRIBUTE_NAME we have to get rid of */
			    	if(regionIterator.hasNext() ) regionIterator.next();
			    	if(regionIterator.hasNext() ) r = ((ITextRegion)regionIterator.next());
	...
	public String getElementAsFlatString(){
		...
		if(fullRegionText==null) return null;
