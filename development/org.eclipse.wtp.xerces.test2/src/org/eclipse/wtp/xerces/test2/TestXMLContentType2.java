 ...
 *******************************************************************************/
package org.eclipse.wtp.xerces.test2;
...
public class TestXMLContentType2 implements IApplication { // extends TestCase
	...
	private static final boolean DO_FAILING_ORDER = false;
	private Boolean applicationResult;


	public TestXMLContentType2() {
		super();
	}
	...
	public boolean testGetParser(int factoryMethod) throws ParserConfigurationException, SAXException {
		...
		switch (factoryMethod) {
			case OSGI_SERVICE :
				factory = WTPTestXercesPlugin2.instance().getFactoryWithOSGiService();
				break;
			case DIRECT_INSTANTIATION :
				factory = WTPTestXercesPlugin2.instance().getFactoryWithDirectInstantiation();
				break;
			case SERVICE_WITH_CONTEXT_CLASSLOADER :
				factory = WTPTestXercesPlugin2.instance().getFactoryWithThreadContextClassloader();
				break;
			default :
