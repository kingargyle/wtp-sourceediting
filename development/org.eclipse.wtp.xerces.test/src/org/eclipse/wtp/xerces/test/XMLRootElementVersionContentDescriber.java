 ...
 * <p>
 * This executable extension supports "element" parameter, that can be
 * specified more than once. If the <code>":-"</code> method is used, then the
 * value is treated as "element" (always just one)
 * </p>
 * <p>
 * The value of "element" is specified using such a format
 * <code>{namespace}name/dtd:version</code>. The namespace or dtd part can be
 * omitted and accepted are values like <code>name/dtd</code>,
 ...
 * </p>
 * <p>
 * If the "element" name part is "*", e.g. <code>{namespace}*</code>, it
 * denotes a wildcard match. If the "element" namespace part is empty, e.g.
 ...
 * {@link XMLRootElementVersionContentDescriber} which doesn't detect
 * namespaces and doesn't allow to specify more than one set of dtds, root
 * element names and namespaces which should be detected.
 * </p>
 * <p>
...
public final class XMLRootElementVersionContentDescriber extends XMLContentDescriber implements IExecutableExtension {
	public XMLRootElementVersionContentDescriber() {
		super();
	}
	...
	/*
	 * (Intentionally not included in javadoc) The top-level elements we are
	 * looking for. This value will be initialized by the
	 * <code>setInitializationData</code> method. If no value is provided,
	 * then this means that we don't care what the top-level element will be.
	 * The list is a collection of <code>QualifiedElement</code>.
	 */
	private QualifiedElement[] elementsToFind = null;

	/*
	 * (Intentionally not included in javadoc) Simple value holder for root
	 * element name, its namespace and dtd.
	 */
	...
	/*
	 * (Intentionally not included in javadoc) Determines the validation
	 * status for the given contents.
	 * 
	 * @param contents the contents to be evaluated
	 * 
	 * @return one of the following:<ul> <li><code>VALID</code></li>,
	 * <li><code>INVALID</code></li>, <li><code>INDETERMINATE</code></li>
	 * </ul>
	 * 
	 * @throws IOException
	 */
	private int checkCriteria(InputSource contents) throws IOException {
		...
		try {
			...
			// we may be handed any kind of contents... it is normal we fail
			// to parse
			return INDETERMINATE;
		}
	...
	public void setInitializationData(final IConfigurationElement config, final String propertyName, final Object data) throws CoreException {
		if (data instanceof String)
			...
			// the describer parameters have to be read again, because
			// "element" parameter can be specified multiple times
