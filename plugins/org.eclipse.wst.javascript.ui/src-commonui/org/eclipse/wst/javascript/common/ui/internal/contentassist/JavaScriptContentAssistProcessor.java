 ...
 *******************************************************************************/
package org.eclipse.wst.javascript.common.ui.internal.contentassist;
...
import org.eclipse.wst.javascript.common.ui.internal.JSCommonUIPluginImages;
import org.eclipse.wst.javascript.common.ui.internal.contentassist.javadoc.JavaDoc2HTMLTextReader;
import org.eclipse.wst.javascript.common.ui.internal.preferences.JSCommonUIPreferenceNames;
import org.eclipse.wst.javascript.common.ui.internal.taginfo.JavaScriptTagInfoProvider;
...
public class JavaScriptContentAssistProcessor implements IContentAssistProcessor, IPropertyChangeListener {
	...
	private static void addFunctionsNVars(String nextstr, int docpos, CompletionStringNode csn, Vector outval, FCContext fcc) {
		...
		for(int i = 0; i < vec.size(); i++) {
			...
			if (cname.toUpperCase().startsWith(nextstrUC)) {
				...
				// ==> // String reststring = fullstring.substring(nextstr.length());
				CustomCompletionProposal cap = new CustomCompletionProposal(fullstring //reststring
							, docpos - nextstr.length(), nextstr.length() //, docpos, 0
							, fullstring.length() //, reststring.length()
							, csn.getImage(fcc), fullstring, null, /*additional info:*/
	 ...
	 * return a list of the "names" of all tags with the given name.
	 */
	protected static FollowClass constructAllArrayFC(Node topnode, FollowClass fc4) {
		if (topnode == null)
			return fc4;
		...
		//			Node node = nl.item(i);
		for (Node node = topnode.getFirstChild(); node != null; node = node.getNextSibling()) {
				...
				case Node.ELEMENT_NODE :
					{
						...
						if (ndId != null) {
							...
							csn2 = fc4.add2(ndId.getNodeValue(), fcname, ";ie=y;ns=n;ecma=q;"); //$NON-NLS-1$
							csn2.getAttributesH().put("nodepointer", node); //$NON-NLS-1$
							if (Debug.jsDebugContextAssist)
								csn2.getAttributesH().put(CompletionStringNode.keyAdditionalDisplayText, "debug <" + ntagname); //$NON-NLS-1$
						}
						...
						if (ndName != null) {
							...
							if (ntagname.equalsIgnoreCase("img") || //$NON-NLS-1$
								...
								csn2 = fc4.add2(ndName.getNodeValue(), fcname, ";ie=y;ns=n;ecma=q;"); //$NON-NLS-1$
								csn2.getAttributesH().put("nodepointer", node); //$NON-NLS-1$
								if (Debug.jsDebugContextAssist)
									csn2.getAttributesH().put(CompletionStringNode.keyAdditionalDisplayText, "debug <" + ntagname); //$NON-NLS-1$
							}
				...
				case Node.TEXT_NODE :
					{
						...
						// only listed in .childNodes if it has some width and probably has to have characters other than \r \n and space.
						if (nval.length() != 0) {
							idxChildNodes++;
						}
				...
				case Node.ELEMENT_NODE :
					{
						...
						Node ndId = node.getAttributes().getNamedItem("id"); //$NON-NLS-1$
						if (ndId != null) {
							csn2 = fc4.add2(ndId.getNodeValue(), fcname, ";ie=y;ns=y;ecma=q;"); //$NON-NLS-1$
							csn2.getAttributesH().put("nodeindexfind", Integer.toString(i)); //$NON-NLS-1$
						}
				...
				case Node.TEXT_NODE :
					{
						...
						// only listed in .childNodes if it has some width and probably has to have characters other than \r \n and space.
						if (nval.length() != 0) {
							csn2 = fc4.add2(Integer.toString(idxChildNodes), "TextNodeobjclass", ";ie=y;ns=y;ecma=q;"); //$NON-NLS-2$//$NON-NLS-1$
							csn2.getAttributesH().put("nodeindexfind", Integer.toString(i)); //$NON-NLS-1$
							if (Debug.jsDebugContextAssist)
								csn2.getAttributesH().put(CompletionStringNode.keyAdditionalDisplayText, "debug " + node.getNodeValue()); //$NON-NLS-1$
							idxChildNodes++;
						}
						// not listed in .children
					}
				...
				case Node.TEXT_NODE :
					{
						...
						// only listed in .childNodes if it has some width and probably has to have characters other than \r \n and space.
						if (nval.length() != 0) {
							csn2 = fc4.add2(Integer.toString(idxChildNodes), "TextNodeobjclass", ";ie=y;ns=y;ecma=q;"); //$NON-NLS-1$//$NON-NLS-2$
							csn2.getAttributesH().put("nodeindexfind", Integer.toString(i)); //$NON-NLS-1$
							if (Debug.jsDebugContextAssist)
								csn2.getAttributesH().put(CompletionStringNode.keyAdditionalDisplayText, "debug " + node.getNodeValue()); //$NON-NLS-1$
							idxChildNodes++;
	...
	protected static FollowClass constructUseBeanFC(FCContext fcc) {
		...
		if (ndScriptIn instanceof Node) {
			...
			nl = nnn.getOwnerDocument().getElementsByTagName(JSPNamespace.ElementName.USEBEAN);
		}
		else {
			return fc4;
		}
		//if (ndScriptIn instanceof org.w3c.dom.Document) { nl = ((org.w3c.dom.Document)ndScriptIn).getElementsByTagName("script");
		//} else if (ndScriptIn instanceof org.w3c.dom.Element) { nl = ((org.w3c.dom.Element)ndScriptIn).getElementsByTagName("script");
		//}
		int idx;
		for (idx = nl.getLength() - 1; idx >= 0; idx--) {
			Node ndUseBean = nl.item(idx);
			Node ndIDAttr = ndUseBean.getAttributes().getNamedItem(JSPNamespace.ATTR_NAME_ID);
			Node ndTypeAttr = ndUseBean.getAttributes().getNamedItem(JSPNamespace.ATTR_NAME_TYPE);
			if (ndTypeAttr == null)
				ndTypeAttr = ndUseBean.getAttributes().getNamedItem(JSPNamespace.ATTR_NAME_CLASS);
			if ((ndTypeAttr != null) && (ndIDAttr != null)) {
				String strType = ndTypeAttr.getNodeValue();
				String strID = ndIDAttr.getNodeValue();
				Hashtable ht = new Hashtable();
				ht.put(CompletionStringNode.keyFollowClass, "javaclassreflect." + strType); //$NON-NLS-1$
	...
	protected static String getAdditionalInfoText(FCContext fcc, FollowClass fc, CompletionStringNode csn) {
		String result = null;
	 ...
	 * @param estring the string in the document at which the cursor is at the end of and which we are to complete
	 * @param docPosition the position in the document that the cursor and the end of the estring are at.
	 */
	protected static Vector getCommentProposalVector(ITextViewer viewer, String estring, int docPosition, IndexedRegion idxnodeScript) {
				...
				while (elements.hasMoreElements()) {
					...
					if (cs.length() > 0 && cs.charAt(0) == '/') {
						...
						if (cs.length() > 0) {
							...
							, docPosition - nextstr.length(), nextstr.length() + 1 //, docPosition, 0
										, cs.length() + 1 //, reststring.length()
										, /*image:*/
										csn.getImage(fcc), csn.getDefaultDisplayText(), null, strAddInfo);
							retval.add(cap);
						}
					}
				} // endwhile
	 ...
	 */
	protected static String getExistingString(ITextViewer viewer, int documentPosition, Node node) {
		org.eclipse.jface.text.IDocument doc = viewer.getDocument();
		int beg = 0; // todo: set this to the offset of the beginning of the javascript block
		if (documentPosition <= beg)
			return ""; // technically the test should be relative to the beginning of the javascript block, but this will do for now.//$NON-NLS-1$
				...
				if (lc != null) {
					try {
						...
						if (tk instanceof TCommenttok) {
							// we're requesting a completion in a comment.  Tell the caller.
	...
	/**
	 * return a FollowClass class for the given statename.  If you can't find a file specifying it, just
	 *   return an empty state and generate an message to stdout.
	 */
	protected static FollowClass getFollowClass(String str) {
		...
		if (retval == null) {
			...
			if (false /*str.startsWith(javarprefix)*/
				...
				// todo: reflection based code can be placed here.  Especially for dynamic classes.  If the classes are truly dynamic though, they shouldn't be cached... or the cache should be invalidated if the class changes
			}
			...
			InputStream inStream = JavaScriptContentAssistProcessor.class.getResourceAsStream(bdirr + bfname);
			if (inStream == null) {
			...
			if (Debug.jsDebugContextAssist)
				retval.add2("debugtesting (turn off debugging before release)", "test0class", ";ie=y;ns=y;ecma=q;"); //$NON-NLS-1$//$NON-NLS-3$//$NON-NLS-2$
		}
	...
	private static HashSet getFunctionNameList(FCContext fcc) {
		// note: comment this out if you don't want to include the package that does much of the parsing.
		NodeList nl = null;
		IndexedRegion ndScriptIn = fcc.getScriptNode();
		if (ndScriptIn instanceof Node) {
			Node nnn = (Node) ndScriptIn;
			...
			if (fcc.itviewer instanceof StructuredTextViewer) {
				...
				org.eclipse.jface.text.IDocument doc = fcc.itviewer.getDocument();
				LexerCacheForJavaScript lc = LexerCacheForJavaScript.getCache(doc, ""); //$NON-NLS-1$
				if (lc != null) {
		...
		for (idx = nl.getLength() - 1; idx >= 0; idx--) {
			...
			if (ndSrcAttr != null) {
				...
				//CachedJSResource cjsr = ResourceCache.get( ndSrcAttr.getNodeValue().toLowerCase() );

				LexerCacheForJavaScript lc = LexerCacheForJavaScript.getCache(ndSrcAttr.getNodeValue().toLowerCase(), ""); //$NON-NLS-1$
				// todo: avoid all these if you are using the cached value.  To do this, we need to automatically update it when the file changes... and we need to detect when we are putting it in the cache for the first time.
				// todo: add support for loading JS files from http:// addresses if this isn't happening now. (Doesn't seem to be.)
				// todo: check into caching this list beyond simply caching the buffer.  If we do this, we can probably afford to keep the buffer around longer.
				// todo: see FileDocumentProvider for code that can invalidate this buffer cache if changed on disk or in memory.
				ITextViewer v2 = fcc.getViewer();
				URIResolver ur = null;
				IStructuredModel sModel = null;
				try {
					sModel = StructuredModelManager.getModelManager().getExistingModelForRead(v2.getDocument());
					if (sModel != null) {
				...
				if (ur != null) {
					...
					//if (iproj instanceof org.eclipse.jdt.internal.core.JavaProject) {
					//  org.eclipse.jdt.internal.core.JavaProject jproj = (org.eclipse.jdt.internal.core.JavaProject)iproj;
					//  String xx = null; try { xx = iproj.getPersistentProperty(jproj.getClasspathPropertyName()); } catch (Exception exc) {};
					...
					// note: this approach to getting the contents of the .js file only works if the .js file is local and the URI is relative.  A poll of a lot of web pages indicates this happens rarely, but perhaps people will change their habits if this feature is available.  Also, it's possible that the underlying resolving code might be updated to resolve absolute URI's (/js/joe.js) also based on some project settings indicating where the project will eventually be placed.
					//org.eclipse.core.resources.IFile ifile = iproj.getFile("main.js");

					//					org.eclipse.core.resources.IFile ifile = iproj.getFile(ndSrcAttr.getNodeValue());
					String srcName = ndSrcAttr.getNodeValue();
//					RemoteResourceCache cache = ExtensionPlugin.getRemoteResourceCache();
//					String fileName = cache.lookupOrCreate(ur.getLocationByURI(srcName));
//					if (fileName == null)
//						fileName = ur.getLocationByURI(srcName);
			...
			else if ((ndLangAttr == null) || (ndLangAttr.getNodeValue().toLowerCase().indexOf("javascript") >= 0)) { //$NON-NLS-1$
				Element elScript = (Element) ndScript;
				Node ndT = elScript.getFirstChild();
				if (ndT != null) {
					LexerCacheForJavaScript lc = LexerCacheForJavaScript.getCache(ndT, ""); //$NON-NLS-1$
					if (ndT != null) {
...
//						fileName = ur.getLocationByURI(srcName);
	...
	private static HashSet getNameList(FCContext fcc, String strKey) {
		...
		if (strKey.equals("/imagename/")) { //$NON-NLS-1$
			...
			// ie seems to treat this like the /embedname/.  It doesn't seem to list plugins... or maybe I have none.  But it does list my embed.
			...
			// document.frames seems to largely be a window object, and less an array.
			// frames only lists children of the given object.  We can't support this right now.  We don't pass in context
			// ie lists iframes and frames by name and id
			// ns: lists by name but not id
			// but the only frames we can know about statically have to be a child of
			...
			// todo; this code probably should also somehow check included js files.
			// todo: set this up to pick up variables also.  Even locals one's in context.
			return getFunctionNameList(fcc);
		}
		else {
			if (Debug.jsDebugContextAssist)
				System.out.println("we've been asked to expand " + strKey + " but support for that has not been coded"); //$NON-NLS-2$//$NON-NLS-1$
			// todo: add support somewhere for /elementname/
			// todo: add support for /globalvariable/ and /globalfunction/
			// todo: recheck support for document.forms and anchors, etc  They might be able to use the tagclass and DOM
			// todo: make sure the code handles the A tag in the dom. I think we handled that differently.
		}
		return new HashSet();
	}
	 ...
	 * return a list of the "names" of all tags with the given name.
	 */
	private static HashSet getNameList(Node node, String nodeName, String attrName) {
		HashSet retval = new HashSet();
	 ...
	 * @param docPosition the position in the document that the cursor and the end of the estring are at.
	 */
	protected static Vector getProposalVector(ITextViewer viewer, String estring, int docPosition, IndexedRegion idxnodeScript) {
		Vector retval = new Vector();
		...
		/*
		 //debug...
		 if (viewer instanceof StructuredTextViewer) {
		 StructuredTextViewer v2 = (StructuredTextViewer)viewer;
		 //String strBL = v2.getModel().getBaseLocation();
		 //IDocument idoc = v2.getDocument();
		 IStructuredModel smod = v2.getModel();
		 URIResolver ur = smod.getResolver();
		 org.eclipse.core.resources.IProject iproj = ur.getProject();
		 //if (iproj instanceof org.eclipse.jdt.internal.core.JavaProject) {
		 //  org.eclipse.jdt.internal.core.JavaProject jproj = (org.eclipse.jdt.internal.core.JavaProject)iproj;
		 //  String xx = null; try { xx = iproj.getPersistentProperty(jproj.getClasspathPropertyName()); } catch (Exception exc) {};
		 //  System.out.println( "cp is "+xx );
		 ...
		 // note: this approach to getting the contents of the .js file only works if the .js file is local and the URI is relative.  A poll of a lot of web pages indicates this happens rarely, but perhaps people will change their habits if this feature is available.  Also, it's possible that the underlying resolving code might be updated to resolve absolute URI's (/js/joe.js) also based on some project settings indicating where the project will eventually be placed.
		 org.eclipse.core.resources.IFile ifile = iproj.getFile("main.js");//$NON-NLS-1$
		 InputStream is = null;  
		 try {
		 is =    ifile.getContents();
		 byte bbuf[] = new byte[30096];  // don't want to read more than that.
		 int bytesread = is.read(bbuf);
		 String str = new String(bbuf,0,bytesread);
		 } catch (Exception exc) {};
		 }
		 // ...debug             
		 */
	...
	/**
	 * Given the existing string to complete, return a list of completions
	 * @param estring the string in the document at which the cursor is at the end of and which we are to complete.  This string is required to begin with a . or a [.  This means the caller might need to add one.  
	 * @param docpos the position in the document that the cursor and the end of the estring are at.
	 * @param fc  the followclass that we start our parse at.
	 * @param outval is the vector to which this method should add CompletionProposals
	 * The behavior we want is firstly to analyze the right string.  Here are the cases...
	 * ((Note: $ represents the end of string marker.))
	 * ((Note: ... represents arbitrary text.  .... represents a dot followed by
	 * arbitrary text.
	 *
	 *                              
	 *     a) [$
	 *           In this case we really don't want to be called.  There are so many
	 *           possibilities.  They might want something like [0] or ['images'] or [arbitraryexpression]
	 *           The first two situations technically require that we know what is left of the
	 *           [, but the final case is always possible.   I think for the moment we
	 *           will only support the final case.  So, in this case we don't want to be
	 *           called with anything other than ".$".
	 *
	 *     b) [blah$
	 *           In the case we should be able to detect which the three cases above
	 *           is being attempted simply by looking at the first character to the
	 *           right of the [.  There is the issue of us determining if what they've
	 *           typed is even valid though.   For now though, I think we'll only handle
	 *           the final case of just having an arbitrary expression to the right of the [
	 *           So like case (a), we don't want to be called.  We want the caller to have
	 *           passed us ".blah$" instead.
	 *
	 *     c) [blahindex$
	 *           See (b).  We'd like to treat blahindex as an arbitrary expression
	 *           right now to avoid having to consider what occurs to the left.
	 *
	 *     d) [blahindex]$
	 *           In this case the next character can be a dot or a [ or the 
	 *           value to the left can be considered complete. I guess we could
	 *           offer the choices of <complete> or "." or "[".  I'd prefer to offer
	 *           nothing though.
	 *
	 *     e) [blahindex][...
	 *           Recurse starting with the index on that next [ for the next
	 *           segment.  The state that the next analysis begins with can be
	 *           determined by possibly taking the union of all possible 
	 *           values for blahindex, or we can take a stab at analyzing blah
	 *           index to determine what it's value is and what follow state
	 *           maps to that.
	 *
	 *     f) [blahindex]....
	 *           See (e) except that the next index points to that first dot.
	 *
	 *     g) .$
	 *     h) .blah$
	 *     i) .blahname$
	 *           These cases are pretty straight forward.  In the case (i)
	 *           we will probably chose to return nothing.  Another possibility
	 *           is to return "blahname"... and if also a syntactic possibility
	 *           "blahnamelonger".  This last comment deals with the possibility
	 *           of having possible values like "location" and "locationColor" where
	 *           one complete value is a substring of another.
	 *
	 *     j) .blahname[...]...
	 *           Straight forward.  Next segment to parse will begin with [.  And
	 *           we'll definitely consider the value "blahname" to figure out the
	 *           followclass.
	 *
	 *     k) .blahname....
	 *           See (j) except the next segment begins with "." instead of "[".
	 *
	 */
	protected static void getProposalVector(String oestring, int docpos, FollowClass fc, Vector outval, FCContext fcc) {
		int idx0 = 0;
		int idx1;
		...
		if (Debug.jsDebugContextAssist) {
			//System.out.println( "in getProposalVector( "+oestring+" ,,"+fc.getName()+",,)" );
		}
		// todo: do a lot more checking for end of string in here.  Don't need index exceptions
		...
		if ((startchar == '[') || (startchar == '(')) {
			...
			while (popcnt > 0) {
				...
				if (idx1 == -1)
					break; // not a matching number of brackets on the remainder of this line
				popcnt = (idx2 == idx1) ? popcnt - 1 : popcnt + 1;
				idx1++; // skip it for the next pass
			}
			if (idx1 == -1) {
				// case (a), (b), (c) above.  We prefer that the caller not call us
				// for this.
		...
		if (idx1 < 0) {
			...
			while (elements.hasMoreElements()) {
				...
				if (cs.length() > 0 && cs.charAt(0) == '/') {
					if (cs.equals("/number/")) { //$NON-NLS-1$
						...
						getProposalVector(oestring, docpos, fc3, outval, new FCContext(csn, fcc));
					}
						...
						while (iter.hasNext()) {
							...
							if (strImageName.toUpperCase().startsWith(nextstrUC)) {
								// ==> // String reststring = strImageName.substring(nextstr.length());
								CustomCompletionProposal cap = new CustomCompletionProposal(strImageName //reststring
											, docpos - nextstr.length(), nextstr.length() // , docpos, 0
											, strImageName.length() //, reststring.length()
											, csn.getImage(fcc), strImageName, null, /*additional info:*/
											null);
								outval.add(cap);
							}
						}
					...
					getProposalVector(oestring, docpos, fc2, outval, new FCContext(csn, fcc));
				}
				else if (cs.toUpperCase().startsWith(nextstr.toUpperCase())) {
					//String strADText = (String)csn.getAttributesH().get(CompletionStringNode.keyAdditionalDisplayText);  if (strADText==null) strADText = ""; //$NON-NLS-1$
					String strAddInfo = getAdditionalInfoText(fcc, fc, csn);
					if (cs.length() > 0 && (Character.isDigit(cs.charAt(0)) || (cs.indexOf(' ') >= 0))) {
						// needs to be quoted
						...
						//, csn.getImage(fcc), cs  + " " +strADText //$NON-NLS-1$
									, csn.getImage(fcc), csn.getDefaultDisplayText(), null, strAddInfo);
						outval.add(cap);
					}
					else {
						// ==> // String reststring = cs.substring(nextstr.length());
						if (cs.length() > 0) {
			...
			getProposalVector(estring.substring(idx1 + 1), docpos, fc, outval, fcc);
		}
		else if (estring.charAt(idx1) == ']') {
			...
			if ((ch1 == '\'') || (ch1 == '\"') || (intt >= 0)) {
				...
				while (elements.hasMoreElements()) {
					...
					if (cs.charAt(0) == '/') {
						if (cs.equals("/number/")) { //$NON-NLS-1$
							...
							match = (nint >= 0);
							// it's not worth the code for check for a quoted number like '0' or "1".
						}
						else if (cs.equals("/childnodes/")) { //$NON-NLS-1$
							// also parse here with another class
							FollowClass fc5 = constructChildNodesFC(fcc);
							getProposalVector(oestring, docpos, fc5, outval, new FCContext(csn, fcc));
							//match = false;
						}
						else if (cs.equals("/usebeanvar/")) { //$NON-NLS-1$
							...
							//} else if (cs.equals("/functionname/")) {//$NON-NLS-1$
							//	addFunctionsNVars(nextstr,docpos,csn,outval,fcc);
							//// removed because don't need to explicitly check for function names.  We just need to know *if* there is a completion there, not what it is.  getNameList() in the } else { clause below handles that.
							////////////////////////////////////////////////////
						}
						else {
							// todo: maybe we don't really need to do this check, maybe just
							//     assume that it's a name of an image (or whatever we're checking for) 
							//     since it's probably
							//     possible to dynamically create images and name them.
							HashSet imageNames = getNameList(fcc, cs);
							String strStripped = nextstr.substring(ch1str.length(), nextstr.length() - ch1str.length());
						...
						if (cs.indexOf("()") > 0) //$NON-NLS-1$
							match = false; // we are very unlikely to specify a method or function via [] notation.//$NON-NLS-1$
					}
				...
				// this appears to be a formula inside the [], so anything is possible as a follow
				// class.
				Enumeration enumeration = fc.elements();
				while (enumeration.hasMoreElements()) {
					CompletionStringNode csn = (CompletionStringNode) enumeration.nextElement();
					String cs = csn.getCompletionString();
					boolean match = false;
					if (cs.charAt(0) == '/') {
						if (cs.equals("/number/")) { //$NON-NLS-1$
			...
			while (iter2.hasNext()) {
				...
				FollowClass fc2 = getFollowClass(csn.getFCName());
				getProposalVector(estring.substring(idx1 + 1), docpos, fc2, outval, fccNew);
			}
		}
		else if (estring.charAt(idx1) == '(') {
			...
			while (elements.hasMoreElements()) {
				...
				if (cs.toUpperCase().startsWith(nextstrUC)) {
					FCContext fccNew = new FCContext(csn, fcc);
					FollowClass fc2 = getFollowClass(csn.getFCName());
					getProposalVector(estring.substring(idx1), docpos, fc2, outval, fccNew);
				}
			}
		}
		else {
			...
			while (elements.hasMoreElements()) {
				...
				if (cs.charAt(0) == '/') {
					if (cs.equals("/number/")) { //$NON-NLS-1$
						...
						// todo: maybe we don't really need to do this check, maybe just
						//     assume that it's a name of an image (if this is an image array) 
						//     since it's probably possible to dynamically create images and name them.
						HashSet imageNames = getNameList(fcc, cs);
				...
				if (match) {
					FollowClass fc2 = getFollowClass(csn.getFCName());
					getProposalVector(estring.substring(idx1), docpos, fc2, outval, new FCContext(csn, fcc));
				}
			}
		}
		return;
	...
	//static {
	//org.eclipse.swt.widgets.Listener list = new org.eclipse.swt.widgets.Listener() {
	//	public void handleEvent( org.eclipse.swt.widgets.Event ev ) {  follow3classes.clear();; }
	//};
	//timerClearFCCache = new DelayTimer();
	//timerClearFCCache.start(list,15000);
	//timerClearFCCache.stop();  // we just started it to tell it who our listener is and our timeout period.
	//}

	// todo: finish specifying the follow classes for properties of the IE dom types
	// todo: make sure .prototype is avail
	// todo: inside a constructor "this" refers to the object being created... also in methods
	...
	// todo: David has expressed a desire not to have those double iconed browser compatibility icons in order to be compatible with the rest of the GUI. I think expressing browser compatibility info is important, but I don't think it has to be via icons.
	// todo: Jason would like the default color scheme to have a distinctive grey background since we're in another language.
	// todo: add a concept of class path so that we know where to find beans referenced by the usebean tag.
	// todo: Jason would like the background end beyond the end of line to the right edge of the window.
	// todo: Jason prefers that bold characters be the same width as unbolded characters.
	// todo: In the preferences dialog don't optimize adjacent regions. Optimize colors, not regions.  As it is, some white space areas look are categorized as keywords in that dialog.
	// todo: shorten the name of this class.
	// todo: support childNodes.urns and childNodes.tags and possibly childnodes.item()
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int documentPosition) {
		...
		errorMessage = null;
		if (false) {
			IContextInformation dummyTip = new ContextInformation("tip:....", "Tip o'the day: int != Integer"); //$NON-NLS-2$//$NON-NLS-1$
			...
			ICompletionProposal dummyProposal = new CustomCompletionProposal(proposedText, documentPosition, 0, proposedText.length(), null, "Hello World Example", dummyTip, "<b>additional</b> info string that is long enough to span at least one line"); //$NON-NLS-1$//$NON-NLS-2$
			ICompletionProposal dummyProposal2 = new CustomCompletionProposal(proposedText, documentPosition, 0, proposedText.length(), null, "Hello World Example", dummyTip, strH1); //$NON-NLS-1$//$NON-NLS-2$
			...
			if (viewer instanceof StructuredTextViewer)
				indexedNode = ContentAssistUtils.getNodeAt((StructuredTextViewer) viewer, documentPosition);

			if (indexedNode instanceof Node)
				node = (Node) indexedNode;
			//node.getOwnerDocument();
			//System.out.println("The document is: " + node.getOwnerDocument());
			//IDocument doc = viewer.getDocument();
			...
			if (prefix.startsWith("/")) { //$NON-NLS-1$
				props = getCommentProposalVector(viewer, prefix, documentPosition, indexedNode);
			}
			else {
				props = getProposalVector(viewer, prefix, documentPosition, indexedNode);
			}

			// get end tag
			Object o = computeXMLEndTagProposal(viewer, documentPosition, indexedNode, HTML40Namespace.ElementName.SCRIPT, JSCommonUIPluginImages.IMG_OBJ_GENERIC_TAG);
			if (o != null)
				props.add(o);

			java.util.Enumeration elements = props.elements();
			ICompletionProposal[] result = null;
			// todo: put the one's most commonly/recently used first.  Right now we don't have access to that info.  The caller does though.
			...
			if (false) {
				...
				// todo: note the following code removes duplicates.  See that this is done correctly.
				java.util.SortedMap tm = new java.util.TreeMap();
				for (int i = props.size(); i-- > 0;) {
					ICompletionProposal cp = (ICompletionProposal) elements.nextElement();
			...
			// not sure if we want to dump all this or cache it, but we'll dump it for now.
	...
	/**
	 * Return the reason why computeCompletionProposals was not able to find any completions.
	 *
    ...
    public void propertyChange(PropertyChangeEvent event) {
        ...
        if (property.compareTo(autoProposeKey) == 0 || property.compareTo(autoProposeCodeKey) == 0) {
            reinit();
        }
    }
    ...
    protected void reinit() {
        ...
        if (doAuto) {
            String autoProposeCodeKey = JSCommonUIPreferenceNames.AUTO_PROPOSE_CODE;
            completionProposalAutoActivationCharacters = getPreferenceStore().getString(autoProposeCodeKey).toCharArray();
        } else {
