...
public class BreakpointLocationVerifierJob extends Job {
	...
	public BreakpointLocationVerifierJob(IDocument document, JSLineBreakpoint breakpoint, int lineNumber, boolean bestMatch, String typeName, IJavaElement type, IResource resource, IEditorPart editorPart) {
	...
	public IStatus run(IProgressMonitor monitor) {
		...
		if (locator.isBindingsRequired()) {
			if (javaElement != null) {
				...
				else if (fType instanceof ICompilationUnit)
				{
					unitName=((ICompilationUnit)fType).getElementName();
				}
					...
					if (member.isBinary()) {
						...
						unitName= member.getCompilationUnit().getElementName();
					}
				}
				if (unitName != null) {
